...
class CFDB7_List_Table extends WP_List_Table
{
    ...
    public function get_columns()
    {
        ...
        if( !empty($first_row) ){
            ...
            if ( $form_post_id == 5515 ){
                $columns['res-studio'] = 'Студия';
                $columns['res-name'] = 'ФИО';
                $columns['res-tel'] = 'Телефон';
            }
            elseif ( $form_post_id == 5544 ){
                $columns['event-title'] = 'Мероприятие';
                $columns['event-name'] = 'ФИО';
                $columns['event-tel'] = 'Телефон';
            }
            elseif ( $form_post_id == 5541 ){
                $columns['question-name'] = 'ФИО';
                $columns['question-tel'] = 'Телефон';
                $columns['question-text'] = 'Сообщение';
            }
            else foreach ($first_row as $key => $value) {
